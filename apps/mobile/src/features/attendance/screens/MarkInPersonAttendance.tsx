    ...
    <SafeAreaView className={`flex-1 ${screenClasses}`}>
      <ScrollView className="flex-1">
        <View className={`px-${isTablet ? '8' : '5'} py-6`}>
          <Text className={`text-2xl font-bold ${titleClasses} mb-1`}>Mark In-Person Attendance</Text>
          ...
          {activeSession ? (
            <View className={`p-5 rounded-2xl border ${cardClasses} mb-6`}>
              ...
              <Text className={`text-sm ${bodyTextClasses} mt-1`}>
                Method: {activeSession.method === 'ble' ? 'BLE Proximity' : activeSession.method === 'pin' ? 'PIN Entry' : 'QR Code'}
              </Text>
            </View>
          ) : (
            <View className={`p-5 rounded-2xl border ${cardClasses} mb-6 items-center`}>
              <Text className={`text-center ${bodyTextClasses}`}>
                No active session. Ask your lecturer to start one.
              </Text>
            </View>
          )}

          {activeSession?.method === 'pin' && (
            <View className="mb-6">

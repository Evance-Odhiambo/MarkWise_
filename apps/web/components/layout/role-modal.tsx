...
import React, { Fragment, useEffect, useRef, useState, Dispatch, SetStateAction } from "react";
...
export const RoleModal: React.FC<RoleModalProps> = ({ isOpen, onClose, mode, onModeChange }) => {
  ...
  // Popover, not a modal: the rest of the page stays interactive (no
  // backdrop, no scroll lock), so it needs its own close triggers instead
  // of "click the overlay" - click outside the panel, or Escape.
  useEffect(() => {
    if (!isOpen) return;
  ...
  return (
    <Fragment>
      <div
        ref={panelRef}
        className="fixed top-16 right-3 sm:top-[4.25rem] sm:right-6 z-50 w-[calc(100vw-1.5rem)] max-w-xs bg-white rounded-2xl shadow-2xl shadow-emerald-900/15 ring-1 ring-slate-900/5 overflow-hidden flex flex-col origin-top-right"
        style={{ animation: "mw-pop 0.2s cubic-bezier(0.34,1.56,0.64,1)" }}
      >
        {/* Top accent bar */}

...
export async function POST(request: Request) {
  try {
    ...
    const { institutionId, staffNumber } = body;

    if (!institutionId || !staffNumber) {
      return NextResponse.json(
        { error: "Institution ID and staff number are required" },
        { status: 400 }
      );
    }
    ...
    try {
      const response = await fetch(
        `${institution.apiUrl}/lecturers?staffNumber=${encodeURIComponent(staffNumber)}`,
        {

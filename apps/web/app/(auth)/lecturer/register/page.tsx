...
import { InstitutionSelector } from "@/components/features/auth/institution-selector";
import { NumberInput } from "@/components/features/auth/number-input";
...
export default function LecturerRegisterPage() {
  ...
  const [verifiedData, setVerifiedData] = useState<{ name: string } | null>(
    null,
  );
  ...
  const handleVerify = async (e: React.FormEvent) => {
    e.preventDefault();
    const normalizedStaffNumber = cleanIdentifier(staffNumber);
    if (!institutionId || !normalizedStaffNumber) {
      setVerificationError(
        "Please select an institution and enter your staff number",
      );
      return;
    }
    ...
    try {
      const response = await fetch("/api/v1/lecturers/verify", {
        method: "POST",
      ...
      if (response.ok && data.valid && data.name) {
        ...
        setVerificationError(
          data.error || "Verification failed - invalid staff number",
        );
      }
    } catch {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/v1/lecturers/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          institutionId,
  ...
  return (
    <AuthLayout
      ...
      title={
        step === "verify" ? "Set up your teaching space" : "Create your account"
      }
      description={
        step === "verify"
          ? "Verify your staff identity first, then create secure access to your teaching tools."
    ...
    >
      ...
      {step === "register" && verifiedData && (
        <form onSubmit={handleRegister} className="space-y-5">
          <div>
            <label className={labelClass}>Username (verified)</label>
            <input
              type="text"

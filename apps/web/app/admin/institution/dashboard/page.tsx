...
import { BookOpen, GraduationCap, LogOut, Settings2, UsersRound } from "lucide-react";
...
export default function InstitutionAdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (!storedUser) {
      router.replace("/admin/system-admin/login");
      return;
    }

    const parsedUser = JSON.parse(storedUser) as StoredUser;
    if (parsedUser.role !== "INSTITUTION_ADMIN") {
      router.replace("/admin/system-admin/institutions");
      return;
    }
    ...
    <main className="min-h-dvh bg-gradient-to-br from-emerald-50 via-white to-cyan-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="flex flex-col gap-4 border-b border-emerald-100 pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700">Institution admin</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Welcome, {user.name ?? "Admin"}</h1>
            <p className="mt-2 text-slate-600">Manage {user.institutionName ?? "your institution"} from one workspace.</p>
          </div>
        ...
        <Card className="border-emerald-100 bg-white/80">
          <CardHeader>
            ...
            <CardDescription>Complete these steps before inviting your teaching team.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3 text-sm text-slate-600 sm:grid-cols-3">
            <p className="rounded-lg bg-emerald-50 p-3">1. Configure academic structure</p>
            <p className="rounded-lg bg-emerald-50 p-3">2. Add lecturers and students</p>
            <p className="rounded-lg bg-emerald-50 p-3">3. Launch your first attendance session</p>
          </CardContent>
        </Card>
      </div>
    </main>

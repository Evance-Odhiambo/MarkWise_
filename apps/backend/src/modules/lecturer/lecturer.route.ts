...
export const lecturerRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(
    ...
    { preHandler: requireAttendanceRole("lecturer") },
    async (request, reply) => {
      const lecturer = await app.prisma.lecturer.findUnique({ where: { id: request.user.id }, select: { institutionId: true } });
      if (!lecturer) return reply.code(404).send({ error: "Lecturer record not found" });

      const units = await app.prisma.unit.findMany({
        where: { semester: { courseYear: { course: { institutionId: lecturer.institutionId } } } },
        orderBy: { code: "asc" },
        select: { id: true, code: true, name: true },
      });
      const assignments = await app.prisma.lecturerUnit.findMany({ where: { lecturerId: request.user.id }, select: { unitId: true } });
      return reply.send({ units, selectedUnitIds: assignments.map(({ unitId }) => unitId) });
    },
  );
  ...
  app.post<{ Body: TeachingUnitSelectionBody }>(
    ...
    async (request, reply) => {
      ...
      if (!unitIds) return reply.code(400).send({ error: "Unit IDs must be an array" });

      const lecturer = await app.prisma.lecturer.findUnique({ where: { id: request.user.id }, select: { institutionId: true } });
      if (!lecturer) return reply.code(404).send({ error: "Lecturer record not found" });
      const validUnits = await app.prisma.unit.findMany({
        where: { id: { in: unitIds }, semester: { courseYear: { course: { institutionId: lecturer.institutionId } } } },
        select: { id: true },
      });
      if (validUnits.length !== unitIds.length) return reply.code(400).send({ error: "One or more units are not part of your institution" });

      await app.prisma.$transaction(async (transaction) => {
        await transaction.lecturerUnit.deleteMany({ where: { lecturerId: request.user.id } });
        if (unitIds.length > 0) await transaction.lecturerUnit.createMany({ data: unitIds.map((unitId) => ({ lecturerId: request.user.id, unitId })) });
      });
  ...
  app.post<{ Body: LecturerRegistrationBody }>("/register", async (request, reply) => {
    ...
    if (!institutionId || !staffNumber || !name || !normalizedEmail || !password) {
    ...
    if (!lecturer) return reply.code(404).send({ error: "Lecturer record not found" });
    if (lecturer.fullName.trim() !== name) return reply.code(400).send({ error: "Name does not match the lecturer record" });
    if (lecturer.auth) return reply.code(409).send({ error: "This lecturer account is already registered" });
    ...
    try {
      ...
      await app.prisma.$transaction(async (transaction) => {
        ...
        await transaction.lecturerAuth.create({
          data: { lecturerId: lecturer.id, email: normalizedEmail, passwordHash },
        });
      });
  ...
  app.get<{ Querystring: { institutionId?: string } }>(
    ...
    async (request, reply) => {
      ...
      if (request.user.role !== 'SUPER_ADMIN' && request.user.institutionId !== institutionId) {
        return reply.code(403).send({ error: "You can only view lecturers for your own institution" });
      }
  ...
  app.post<{ Body: BulkCreateBody }>('/', { preHandler: requireRoles('SUPER_ADMIN', 'INSTITUTION_ADMIN') }, async (request, reply) => {
    const { institutionId, lecturers } = request.body;
    if (!institutionId || !Array.isArray(lecturers)) {
      return reply.code(400).send({ error: "Institution ID and lecturers are required" });
    }

    if (request.user.role !== 'SUPER_ADMIN' && request.user.institutionId !== institutionId) {
      return reply.code(403).send({ error: "You can only save lecturers for your own institution" });
    }
  ...
  app.post<{ Body: ImportRequest }>(
    ...
    async (request, reply) => {
      try {
        const institutionId = (request.body as { institutionId?: string }).institutionId;
        if (request.user.role !== 'SUPER_ADMIN' && request.user.institutionId !== institutionId) {
          return reply.code(403).send({ error: "You can only import lecturers for your own institution" });
        }

...
export const adminRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{ Body: OnboardingBody }>("/onboarding", async (request, reply) => {
    ...
    if (existing) {
      return reply
        .code(409)
        .send({ error: "An onboarding request is already pending" });
    }
    ...
    async (request, reply) => {
      ...
      if (!contactName || !email || !institutionName || !password) {
        return reply
          ...
          .send({
            error:
              "Contact name, email, institution name, and password are required",
          });
      }
      ...
      if (!onboardingRequest?.institutionId) {
        return reply
          .code(403)
          .send({ error: "This institution request has not been approved" });
      }

      const existingAdmin = await app.prisma.admin.findUnique({
        where: { email },
        select: { id: true },
      });
      if (existingAdmin) {
  ...
  app.post<{ Params: ApprovalParams; Body: ApprovalBody }>(
    ...
    async (request, reply) => {
      ...
      if (!onboardingRequest || onboardingRequest.status !== "PENDING") {
        return reply
          .code(404)
          .send({ error: "Pending onboarding request not found" });
      }

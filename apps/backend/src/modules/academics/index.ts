    ...
    async (request, reply) => {
      ...
      const courses: ImportedCourse[] = rawCourses.flatMap(
        (rawCourse, courseIndex) => {
          ...
          if (!name) return [];
          const years = Array.isArray(course.years)
            ? course.years.filter((year): year is ImportedYear =>
                Boolean(year && typeof year === "object"),
              )
            : [];
          return [
            {
              id: `imported-course-${courseIndex}`,
              name,
              duration: years.length || 1,
              years,
            },
          ];
        },
            ...
            (yearTotal, year) =>
              ...
              (year.semesters ?? []).reduce(
                (semesterTotal, semester) =>
                  semesterTotal + (semester.units?.length ?? 0),
                0,
              ),
            0,
          ),
